...
export function compressImage(file: File, maxWidth = 800, quality = 0.7): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      compressDataUrl(event.target?.result as string, maxWidth, quality)
        .then(resolve)
        .catch(reject);
    };
...
export function compressDataUrl(dataUrl: string, maxWidth = 800, quality = 0.7): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Output as WebP for best compression, or JPEG if WebP isn't supported
      const compressedDataUrl = canvas.toDataURL('image/webp', quality);
      resolve(compressedDataUrl);
    };
    img.onerror = (error) => resolve(dataUrl); // fallback to original if image fails to load
  });
}
